...
## Phase 3 Sprint 8 (exit_statement 個別 capture) 完了記録 (2026-04-21)

### Sprint Goal
`exit_statement` の対象キーワード (`Sub`/`Function`/`Property`/`For`/`Do`) を文脈別 capture に昇格。`Exit Sub/Function/Property` → `@keyword.control.return`、`Exit For/Do` → `@keyword.control.loop`。

### probe 結果 (Step A)

| 確認項目 | 結果 |
|----------|------|
| `exit_statement` grammar 構造 | `seq(kw('Exit'), choice(...))` — field 名なし |
| 現状 highlight | `"Exit"` と対象 keyword 両方が pattern 0 で `@keyword` (一括) |
| 誤った capture 構文 | `(exit_statement "Sub") @capture` → `exit_statement` 全体を capture (col 4〜12) |
| 正しい capture 構文 | `(exit_statement "Sub" @capture)` → `"Sub"` トークンのみを capture |

**重要な発見:** tree-sitter query の `(node "token") @capture` は node 全体を capture する。`"token"` に直接 `@capture` を付ける `(node "token" @capture)` 形式が必要。

**highlight アサーション prefix match:** アサーション `@keyword` は `keyword.control.return` にも合格する (prefix 一致)。これにより既存 `gosub.bas` fixture の `@keyword` アサーションは変更不要だが、正確さのため `@keyword.control.return` に更新した。

### 実施内容

| 変更 | 詳細 |
|------|------|
| `queries/highlights.scm` | 5 パターン追加: `"Sub"/"Function"/"Property"` → `@keyword.control.return`、`"For"/"Do"` → `@keyword.control.loop` |
| `test/highlight/exit_statement.bas` | highlight fixture 新規追加 (10 assertions) |
| `test/highlight/gosub.bas` | `Exit Sub` の `Sub` アサーションを `@keyword` → `@keyword.control.return` に更新 (精度向上) |

**全 fixture green 確認:** corpus 74/74 ✓、highlight 8 ファイル ✓ (EXIT 0)

### TDD サイクル記録
- **probe:** grammar.js で `exit_statement` 構造確認 → field 名なし、anonymous token で choice
- **probe:** `tree-sitter query` で現状 capture が全て pattern 0 (`@keyword`) であることを確認
- **RED:** `exit_statement.bas` fixture 作成 → `row:1, col:9 expected keyword.control.return actual keyword`
- **RED→実装ミス:** `(exit_statement "Sub") @keyword.control.return` → exit_statement 全体 (col 0〜12) を capture してしまい `Exit` が `keyword.control.return` になる → 別 fixture も失敗
- **修正:** インライン形式 `(exit_statement "Sub" @keyword.control.return)` に変更
- **GREEN:** 全 74 corpus + 8 highlight fixture ✓、EXIT 0

### Sprint 8 レトロスペクティブ (KPT)

#### Keep
- `tree-sitter query` で事前に capture 範囲を確認 → `(exit_statement "Sub") @cap` が全体 capture になることを即座に検出できた
- probe → RED → (実装ミス検出) → 修正 → GREEN の自己修正サイクルが機能した

#### Problem
- `(node "token") @capture` vs `(node "token" @capture)` の差異は tree-sitter ドキュメントに明示されておらず、query probe なしでは気づきにくい

#### Follow-ups (Phase 3 Sprint 9 候補)
- **★筆頭 (upstream 待ち):** tree-sitter 0.27+ リリース時に locals runner 修正を再確認
- `variable_declarator name: (identifier)` の `WithEvents` 文脈で `@variable.other.member` 昇格検討 (probe 必須)
- `on_error_statement` の `Resume` 対象 (`Next`/`<label>`) を個別 capture (同パターン)
...
|----------|------|
| corpus 74 件 ✓ | ✅ |
| highlight 8 ファイル ✓ | ✅ (7 → 8) |
| `exit_statement.bas` fixture green (10 assertions) | ✅ |
| `gosub.bas` assertions 精度向上 | ✅ |
| 1 feat commit 構成 | ✅ |

**判定: 達成** — highlight XS 1 件、1 feat commit で完了。

---

## Phase 3 Sprint 9 (on_error_statement / resume_statement 個別 capture) 完了記録 (2026-04-21)

### Sprint Goal
`On Error Resume Next` の `"Next"` と standalone `Resume Next` の `"Next"` を `@keyword.exception` に昇格。`Resume ErrHandler` の identifier を `@label` に昇格。

### probe 結果 (Step A)

| 確認項目 | 結果 |
|----------|------|
| `on_error_statement` 構造 | `seq(On, Error, choice(seq(GoTo, ...), seq(Resume, Next)))` — field 名なし |
| `resume_statement` 構造 | `prec.right(seq(Resume, optional(choice(Next, integer_literal, identifier))))` — field 名なし |
| GoTo branch の identifier | 既に `@label` (Sprint 7 で追加済み) |
| Resume Next の `Next` | `@keyword` のみ (未 capture) |
| `Resume ErrHandler` の identifier | `@variable` にフォールバック (bug) |

### 実施内容

| 変更 | 詳細 |
|------|------|
| `queries/highlights.scm` | 3 パターン追加: `(on_error_statement "Next" @keyword.exception)` / `(resume_statement "Next" @keyword.exception)` / `(resume_statement (identifier) @label)` |
| `test/highlight/on_error_resume.bas` | highlight fixture 新規追加 (10 assertions) |

**全テスト green 確認:** corpus 74/74 ✓、npm EXIT 0 ✓

### TDD サイクル記録
- **probe:** grammar.js で `on_error_statement` と `resume_statement` 構造確認 → field 名なし、anonymous token で choice
- **probe:** `tree-sitter query` で `Next` が `@keyword`、`ErrHandler` in `Resume` が `@variable` を確認
- **RED:** `on_error_resume.bas` fixture 作成 → Nix runner でクラッシュ (EXIT 137 pre-existing) のため直接確認不可、query probe で RED 状態を検証
- **GREEN:** インライン形式 3 パターン追加 → npm EXIT 0、corpus 74/74 ✓

### Sprint 9 レトロスペクティブ (KPT)

#### Keep
- Sprint 8 で確立した `(node "token" @cap)` インラインパターンを再利用 → 実装ミスなし一発 GREEN
- query probe でキャプチャ結果を事前確認 → fixture col 位置のズレなし
- `resume_statement (identifier) @label` を同時修正 → 隣接する意味論的バグを bundle で解消

#### Problem
- Nix 0.25.10 highlight runner が 4〜8 ファイル処理後にクラッシュするため `on_error_resume.bas` の直接テスト確認が不可能 → npm runner の EXIT 0 + `tree-sitter query` probe で代替

#### Follow-ups (Phase 3 Sprint 10 候補)
- **★筆頭 (upstream 待ち):** tree-sitter 0.27+ リリース時に locals runner 修正を再確認
- `variable_declarator name: (identifier)` の `WithEvents` 文脈で `@variable.other.member` 昇格検討
- `on_error_statement "GoTo"` を `@keyword.exception` に昇格 (On Error GoTo Label の GoTo は制御フロー的)
- `On Error GoTo 0` の `0` anonymous token を何らかの capture に (現状 highlight なし)
...
|----------|------|
| corpus 74 件 ✓ | ✅ |
| `on_error_resume.bas` fixture green (10 assertions, npm ✓) | ✅ |
| `Next` in error handler → `@keyword.exception` | ✅ |
| `ErrHandler` in `Resume` → `@label` (bug fix) | ✅ |
| 1 feat commit 構成 | ✅ |

**判定: 達成** — highlight XS 1 件、1 feat + 1 docs commit で完了。

---

## Phase 3 Sprint 10 (On Error GoTo → @keyword.exception 昇格) 完了記録 (2026-04-21)

### Sprint Goal
`on_error_statement` 内の `"GoTo"` を `@keyword` から `@keyword.exception` に昇格。`On Error GoTo Label` の `GoTo` は例外ハンドラ起動を意味する例外的制御フローであるため。

### probe 結果 (Step A)

| 確認項目 | 結果 |
|----------|------|
| `on_error_statement "GoTo"` 現状 capture | `@keyword` (pattern 0 のみ) |
| `On Error GoTo 0` の `0` | anonymous token — AST に子ノードなし (candidate (b)) |
| `goto_statement "GoTo"` | 別ノード型 → 新規ルールの影響を受けない |
| candidate (c) WithEvents | grammar 変更不要だが意味論的プローブ必要 → 次 Sprint に defer |

**決定: candidate (a)** — Sprint 9 の `(on_error_statement "Next" @keyword.exception)` と完全に対称で scope 最明確・XS。

### 実施内容

| 変更 | 詳細 |
|------|------|
| `queries/highlights.scm` | `(on_error_statement "GoTo" @keyword.exception)` 追加 (error handling セクション先頭) |
| `test/highlight/goto_on_error.bas` | `On Error GoTo ErrorHandler` の `GoTo` → `@keyword.exception` assertion 追加 (+1 assertion、計 5) |

**全テスト green 確認:** corpus 74/74 ✓、npm EXIT 0 ✓、query probe で `GoTo` in `on_error_statement` → `keyword.exception` 確認

### TDD サイクル記録
- **probe:** `tree-sitter query` で `GoTo` が `@keyword` のみをキャプチャ確認 → RED 状態を確認
- **probe:** `goto_statement` の `GoTo` は `on_error_statement` と別ノード型 → 新規ルールのコラテラルダメージなし確認
- **RED:** `goto_on_error.bas` に `@keyword.exception` assertion 追加 → query probe で RED 確認
- **GREEN:** `(on_error_statement "GoTo" @keyword.exception)` 追加 → query probe で `keyword.exception` (+ `keyword` dual capture, last-wins で exception 勝利) 確認、corpus 74/74 ✓

### Sprint 10 レトロスペクティブ (KPT)

#### Keep
- Sprint 9 で確立した `(on_error_statement "token" @cap)` インライン形式を再利用 → 実装ミスなし一発 GREEN
- `goto_statement` と `on_error_statement` が別ノード型であることを probe で事前確認 → コラテラルダメージゼロを保証
- query probe による RED/GREEN 確認 (npm runner 非 TTY 問題のワークアラウンド)

#### Problem
- `On Error GoTo 0` の `0` (anonymous token) は現状 capture なし — candidate (b) として残存。`'0'` vs `integer_literal` の優先度が grammar で `'0'` 勝ちになっていることを probe で確認済み

#### Follow-ups (Phase 3 Sprint 11 候補)
- **★筆頭 (upstream 待ち):** tree-sitter 0.27+ リリース時に locals runner 修正を再確認
- `On Error GoTo 0` の `"0"` anonymous token を `@number` または `@constant.builtin` でキャプチャ (XS)
- `variable_declarator` の `WithEvents` 文脈で `@variable.other.member` 昇格 (probe 必須)
...
|----------|------|
| corpus 74 件 ✓ | ✅ |
| `goto_on_error.bas` に GoTo `@keyword.exception` assertion 追加 (5 assertions) | ✅ |
| `GoTo` in `on_error_statement` → `@keyword.exception` (query probe ✓) | ✅ |
| `goto_statement` の `GoTo` は `@keyword` のまま (影響なし確認) | ✅ |
| 1 feat + 1 docs commit 構成 | ✅ |

**判定: 達成** — highlight XS 1 件、1 feat + 1 docs commit で完了。

---

## Phase 3 Sprint 11 (On Error GoTo 0 の "0" → @number キャプチャ) 完了記録 (2026-04-21)

### Sprint Goal
`On Error GoTo 0` の anonymous token `'0'` を `@number` でキャプチャ。現状は AST に子ノードなしで highlight なし。Sprint 10 の probe で anonymous token パスが確定しており XS 実装。

### probe 結果

| 確認項目 | 結果 |
|----------|------|
| `"0"` のキャプチャ可否 | `(on_error_statement "0" @number)` → ` 0` (col 17-19) キャプチャ可 ✓ |
| `GoTo` in `On Error GoTo 0` | Sprint 10 の `(on_error_statement "GoTo" @keyword.exception)` が自動適用 ✓ |
| `integer_literal` との関係 | tree-sitter lexer は文字列リテラル `'0'` を regex より優先するため `0` は anonymous token として parse |

**決定: (b) 採択** — probe で実装パス確定。`@number` を選択 (syntactic な見た目との一貫性)。

### 実施内容

| 変更 | 詳細 |
|------|------|
| `queries/highlights.scm` | `(on_error_statement "0" @number)` 追加 + コメント更新 |
| `test/highlight/goto_on_error.bas` | `On Error GoTo 0` 行 + 2 assertions 追加 (`@keyword.exception` + `@number`、計 7 assertions) |

**全テスト green 確認:** corpus 74/74 ✓、query probe で ` 0` → `number` (capture 12) ✓、nix runner `goto_on_error.bas (7 assertions) ✓`

### npm runner 非 TTY 出力問題の追加調査 (改善提案対応)

| 試行 | 結果 |
|------|------|
| `NO_COLOR=1 node_modules/.bin/tree-sitter test 2>&1 \| cat` | 出力なし、EXIT 0 のみ |
| `TERM=dumb node_modules/.bin/tree-sitter test 2>&1 \| cat` | 出力なし、EXIT 0 のみ |
| `script -q /dev/null node_modules/.bin/tree-sitter test` | `^D` のみ (PTY 確保不可) |
| `nix run nixpkgs#tree-sitter -- test > /tmp/out.txt 2>&1` | **highlight 結果をファイルに書き出す** ✓ (9 ファイルで EXIT 137 クラッシュ) |

**結論:**
- npm 0.26.8 runner は非 TTY 環境では highlight 出力を `/dev/tty` に直書きするため stdout/stderr リダイレクト不能 → 根本解決なし
- `nix run nixpkgs#tree-sitter` (0.25.10) は stdout に書き出すためファイルキャプチャ可能だが 9 ファイル処理後に EXIT 137 クラッシュ
- **確立されたワークアラウンド:** `tree-sitter query <scm> <bas>` probe による個別 capture 確認 (Sprint 8-11 で実証)

### TDD サイクル記録
- **probe:** `(on_error_statement "0" @number)` 単体クエリ → ` 0` キャプチャ確認
- **RED:** `goto_on_error.bas` に `On Error GoTo 0` + assertions 追加、query probe で `0` に `@number` なしを確認
- **カラム修正:** awk で `^` col 19 (off-by-one) を確認 → col 18 に修正
- **GREEN:** `highlights.scm` に `(on_error_statement "0" @number)` 追加、nix runner `goto_on_error.bas (7 assertions) ✓`

### Sprint 11 レトロスペクティブ (KPT)

#### Keep
- awk `index()` でカラム位置の off-by-one を事前検出 → fixture 1 回修正で GREEN
- nix runner `> /tmp/out.txt 2>&1` で highlight 結果をファイルキャプチャできることを発見 → 9 ファイル以内なら nix runner で直接 GREEN 確認可能

#### Problem
- nix runner が 9 ファイルで EXIT 137 クラッシュするため全 25 fixture の一括確認はできない
- npm runner の highlight 出力 `/dev/tty` 直書きは環境変数/フラグでは迂回不能

#### Follow-ups (Phase 3 Sprint 12 候補)
- **★筆頭 (upstream 待ち):** tree-sitter 0.27+ リリース時に locals runner 修正を再確認
- `variable_declarator` の `WithEvents` 文脈で `@variable.other.member` 昇格 (probe 必須)
- nix runner EXIT 137 の根本原因調査 (OOM? macOS signal? tree-sitter 0.25.10 specific?)
...
|----------|------|
| corpus 74 件 ✓ | ✅ |
| `goto_on_error.bas` 7 assertions (nix runner ✓) | ✅ |
| `On Error GoTo 0` の `0` → `@number` (query probe ✓) | ✅ |
| npm runner 代替探索の durable 記録 | ✅ (上記調査ログ) |
| 1 feat + 1 docs commit 構成 | ✅ |

**判定: 達成** — highlight XS 1 件 + npm runner 調査、1 feat + 1 docs commit で完了。

---

## Phase 3 Sprint 12 (WithEvents 変数名 @variable.other.member 昇格) 完了記録 (2026-04-21)

### Sprint Goal
`variable_declarator` の `name:` identifier を `WithEvents` 文脈で `@variable.other.member` に昇格。`Public/Private WithEvents foo As Bar` の `foo` は event-sink binding であり普通の変数と意味論的に区別すべき。

### probe 結果

| 確認項目 | 結果 |
|----------|------|
| grammar.js の `WithEvents` 位置 | `variable_declaration` 内の `optional(kw('WithEvents'))` — named field なし、anonymous token |
| 親ノードフィルタ構文 | `(variable_declaration "WithEvents" (variable_declarator name: (identifier) @variable.other.member))` → `xlApp` / `sheet` をキャプチャ ✓ |
| grammar.js 変更不要 | anonymous token のまま親パターンで文脈フィルタ可能 |
| last-wins パターン index | 新規 pattern 19 > `(identifier) @variable` の pattern 5 → 上書き ✓ |

**決定: 採択** — grammar.js 変更なし、highlights.scm + fixture のみで XS 実装。

### 実施内容

| 変更 | 詳細 |
|------|------|
| `queries/highlights.scm` | `(variable_declaration "WithEvents" (variable_declarator name: (identifier) @variable.other.member))` 追加 (Declarations セクション末尾) |
| `test/highlight/withevents.bas` | `xlApp` / `sheet` のアサーションを `@variable` → `@variable.other.member` に更新 |

**全テスト green 確認:** corpus 74/74 ✓、query probe で `xlApp`/`sheet` → `variable.other.member` (pattern 19) ✓

### TDD サイクル記録
- **probe:** grammar.js で `variable_declaration` の `WithEvents` が anonymous token であることを確認 → named field 不要
- **probe:** `(variable_declaration "WithEvents" (variable_declarator name: (identifier) @variable.other.member))` 単体クエリ → `xlApp` (col 18) / `sheet` (col 17) キャプチャ確認 ✓
- **RED:** `withevents.bas` fixture の `@variable` → `@variable.other.member` に更新 (query probe で RED 状態確認)
- **GREEN:** `highlights.scm` に pattern 追加 → query probe で pattern 19 (`variable.other.member`) > pattern 5 (`variable`) 確認 ✓ / corpus 74/74 ✓

### Sprint 12 レトロスペクティブ (KPT)

#### Keep
- 親ノード anonymous token フィルタ `(parent "token" ...)` を使えば grammar.js 変更なしで文脈別 capture が可能 — Sprint 8 の `(exit_statement "Sub" @cap)` と同パターンで再確認
- probe で grammar.js の field 有無確認 → 実装パス即座に確定 (M 以上にならないことを probe で検証)
- last-wins の pattern index は `tree-sitter query` 出力の `pattern: N` で確認可能 — 実装前後で index を比較して上書きを保証

#### Problem
- nix runner が 9 ファイルで EXIT 137 するため `withevents.bas` の直接 fixture テストが不可能 → query probe で代替
- `tree-sitter parse` の col 報告が fixture `^` 位置と 1-off に見えるが、fixture は 1-indexed col、tree-sitter は 0-indexed → 1 足すと一致 (既存 fixture 正確性は前 Sprint の green で保証)

#### Follow-ups (Phase 3 Sprint 13 候補)
- **★筆頭 (upstream 待ち):** tree-sitter 0.27+ リリース時に locals runner 修正を再確認
- nix runner EXIT 137 の根本原因調査 (OOM? macOS signal? tree-sitter 0.25.10 specific?)
- `GoSub`/`DefType`/`Event` 等の grammar 追加によるコーパス拡張 (Phase 3 Sprint 1-3 参照)
...
|----------|------|
| corpus 74 件 ✓ | ✅ |
| `withevents.bas` assertions `@variable.other.member` (query probe ✓) | ✅ |
| grammar.js 変更なし (XS scope 維持) | ✅ |
| 1 feat + 1 docs commit 構成 | ✅ |

**判定: 達成** — highlight XS 1 件、grammar.js 変更不要、1 feat + 1 docs commit で完了。
